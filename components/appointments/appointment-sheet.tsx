...
type Mode = 'detail' | 'reschedule';

type ConfirmAction =
  | { kind: 'complete' }
  | { kind: 'noShow' }
  | { kind: 'cancel' }
  | { kind: 'reschedule'; iso: string };
              ...
              <>
                ...
                <div className="flex gap-2">
                  <a
                    ...
                    className="border-line hover:bg-muted/50 active:bg-muted/50 bg-card flex h-11 flex-1 items-center justify-center gap-2 rounded-[10px] border text-[13.5px] font-semibold transition-colors"
                  >
                    <Phone
                ...
                {isActive && (
                  <div className="border-sep flex flex-col gap-3 border-t pt-4">
                    ...
                    {started && (
                      <Button
                        variant="outline"
                        onClick={() => setConfirm({ kind: 'noShow' })}
                        disabled={pending}
                        className="h-12 rounded-[12px]"
                      >
                        ...
                        <div className="flex flex-wrap gap-2">
                          {day.slots.map((iso) => (
                            <Button
                              ...
                              disabled={pending}
                              onClick={() =>
                                setConfirm({ kind: 'reschedule', iso })
                              }
      ...
      >
        <DialogContent>
          ...
          {confirm?.kind === 'complete' && (
            <>
              <DialogHeader>
                <DialogTitle>{t.appointment.completeTitle}</DialogTitle>
                <DialogDescription>
                  {t.appointment.completeBody}
                </DialogDescription>
              </DialogHeader>
              <DialogFooter>
                <Button
          ...
          {confirm?.kind === 'noShow' && (
            <>
              <DialogHeader>
                <DialogTitle>{t.appointment.noShowTitle}</DialogTitle>
                <DialogDescription>
                  {t.appointment.noShowBody}
                </DialogDescription>
              </DialogHeader>
              <DialogFooter>
                <Button
          ...
          {confirm?.kind === 'reschedule' && (
            <>
              <DialogHeader>
                <DialogTitle>
                  {t.appointment.rescheduleConfirmTitle}
                </DialogTitle>
                <DialogDescription>
                  {t.appointment.rescheduleConfirmBody(
                    formatTime(new TZDate(new Date(confirm.iso), timezone)),
                  )}
                </DialogDescription>
              </DialogHeader>
              <DialogFooter>

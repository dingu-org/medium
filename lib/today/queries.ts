...
export async function getTodaySnapshot(
  ...
  // Reminders are parked — see lib/reminders/flag.ts. With the feature off
  // nothing schedules or sends one, so every `reminder_jobs` row is a leftover
  // from before the switch. Skip the attention query outright rather than
  // filtering its result, so no `kind: 'reminder'` row can reach the client and
  // no reminder state leaks into an appointment card.
  ...
  const todayAppointments = appointmentRows.map((row) =>
    appointmentView(row, timezone, zonedNow, showReminders),
  );
  ...
  for (const row of reminderRows) {
    ...
    attention.push({
      ...
      customerName: privacyName(row.customerName),
      conversationId: row.conversationId,
      appointment: appointmentView(row, timezone, zonedNow, showReminders),
    });
  }

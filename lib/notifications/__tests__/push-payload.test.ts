...
const escalated: PushEvent = {
  ...
  data: { accountId: PT, conversationId: CONV, customerId: PATIENT },
};
...
const revoked: PushEvent = {
  ...
  data: { accountId: PT, connectionId: CONN, reason: 'unauthorized' },
};
const reminderFailed: PushEvent = {
  name: 'reminder.failed',
  data: { accountId: PT, appointmentId: APPT, reason: 'window_closed' },
};
const EXPIRES = '2026-08-01T00:00:00.000Z';
const renewalDay5: PushEvent = {
  name: 'billing.renewal_due',
  data: { accountId: PT, expiresAt: EXPIRES, daysLeft: 5, offset: 5 },
};
const renewalDay0: PushEvent = {
  name: 'billing.renewal_due',
  data: { accountId: PT, expiresAt: EXPIRES, daysLeft: 0, offset: 0 },
};
const graceStarted: PushEvent = {
  name: 'billing.grace_started',
  data: { accountId: PT, expiresAt: EXPIRES },
};
const downgraded: PushEvent = {
  name: 'billing.downgraded',
  data: { accountId: PT, from: 'solo', to: 'free' },
};
...
describe('buildPushPayload', () => {
  it('deep-links appointment events to the calendar with a dedupe tag', () => {
    const p = buildPushPayload(booked, { customerName: NAME, timezone: TZ });
    ...
    expect(buildPushPayload(cancelled, { customerName: NAME, timezone: TZ })?.tag).toBe(
      `appointment-${APPT}-cancelled`,
    );
    expect(
      buildPushPayload(rescheduled, { customerName: NAME, timezone: TZ })?.tag,
    ).toBe(`appointment-${APPT}-rescheduled`);
  });

  it('keeps the customer name out of the title (iOS lock-screen privacy)', () => {
    for (const event of [booked, cancelled, rescheduled, escalated, needsReply]) {
      const p = buildPushPayload(event, { customerName: NAME, timezone: TZ });
      expect(p?.title).not.toContain(NAME);
    }
  });

  it('nudges a manual reply to the thread with a per-conversation dedupe tag', () => {
    const p = buildPushPayload(needsReply, { customerName: NAME, timezone: TZ });
    expect(p).toMatchObject({
      url: `/chat/${CONV}`,
    ...
    expect(p?.body).toContain(NAME);
    // Falls back to the neutral label when the customer is unknown.
    expect(
      buildPushPayload(needsReply, { timezone: TZ })?.body,
    ).toContain('Një klient');
  });

  it('falls back to a neutral name when the customer is unknown', () => {
  ...
  it('deep-links conversation events to the thread', () => {
    expect(
      buildPushPayload(escalated, { customerName: NAME, timezone: TZ }),
    ).toMatchObject({
      url: `/chat/${CONV}`,
      tag: `conversation-${CONV}-escalated`,
    });
    expect(
      buildPushPayload(resumeOffered, { customerName: NAME, timezone: TZ })?.url,
    ).toBe(`/chat/${CONV}`);
  });

...
| `conversations`        | unique on `(customer_id, channel)`; `handoff_offer_message_id` is dead since 2026-08-30 and dropped by its own later migration; `ai_paused_until` has a partial index for the pause sweep; `last_inbound_at` has two composite indexes for the chat list                                                                                                                                                                                                  |
...
Four references are deliberately not foreign keys, because a dangling value is the correct outcome once retention or erasure removes the target: `messages.source_event_id`, `messages.template_id`, `conversation_days.first_message_id`, and `erasure_archive.account_id`. (`conversations.handoff_offer_message_id` was a fifth until the anchor it held was deleted; the column is dropped by its own migration.)

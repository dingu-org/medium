...
const CalWeek = () => (
  <Screen>
    ...
    <Body pad="14px 18px 24px">
      <div style={{ display: "flex", flexDirection: "column", gap: 18 }}>
        {WEEK_DATA.map((d, i) => (
          <div key={i}>
            <div style={{ fontSize: 11, fontWeight: 700, letterSpacing: "0.05em", textTransform: "uppercase", color: d.today ? MT.brand : MT.ink3, marginBottom: 8, fontFamily: MT.mono }}>{d.label}{d.today && " · sot"}</div>
            {d.appts.length === 0
              ? <div style={{ fontSize: 13, color: "#c2c8d2", padding: "2px 2px 0" }}>— i lirë</div>
...
const CalMonth = () => (
  <Screen>
    <CalHeader view="month" title="Maj 2026" sub="" strip={null} />
    <Body pad="16px 18px 24px">
  ...
  <Screen>
    ...
    <Body pad="18px 18px 24px">
      <div style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", height: "100%", textAlign: "center", padding: "0 28px" }}>
        ...
        <Btn kind="tinted" icon="plus" style={{ marginTop: 16, height: 42 }}>Takim i ri</Btn>
      </div>
    </Body>
    <Fab />
    <Tabs active="calendar" />

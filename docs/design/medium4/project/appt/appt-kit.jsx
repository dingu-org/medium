...
const SInput = ({ value, placeholder, mono, focus }) => (
  <div style={{ height: 46, padding: "0 14px", border: `1px solid ${focus ? MT.brand : MT.line}`, borderRadius: 10, background: "#fff", display: "flex", alignItems: "center", fontSize: 14.5, color: value ? MT.ink : "#a4abb7", fontFamily: mono ? MT.mono : MT.sans, boxShadow: focus ? "0 0 0 3px rgba(59,91,254,0.16)" : "none" }}>{value || placeholder}</div>
);
...
const SlotChip = ({ t, sel }) => (
  <button style={{ padding: "9px 14px", borderRadius: 10, border: `1px solid ${sel ? MT.brand : MT.line}`, background: sel ? MT.brandTint : "#fff", color: sel ? MT.brandInk : MT.ink, fontFamily: MT.mono, fontSize: 13.5, fontWeight: 600, cursor: "pointer", boxShadow: sel ? "0 0 0 3px rgba(59,91,254,0.12)" : "none" }}>{t}</button>
);

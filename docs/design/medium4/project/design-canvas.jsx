...
function DesignCanvas({ children, minScale, maxScale, style }) {
  ...
  // Deep-link: focus the artboard named in location.hash (e.g.
  // #sectionId/artboardId), and follow hashchange — lets other pages (the
  // flow map) link straight through to one screen. Ignores unknown ids.
  React.useEffect(() => {
    if (!ready) return;
    const applyHash = () => {
      const raw = decodeURIComponent((location.hash || '').replace(/^#/, '')).trim();
      if (raw && registry[raw]) api.setFocus(raw);
    };
    applyHash();

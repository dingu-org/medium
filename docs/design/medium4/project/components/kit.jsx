...
if (typeof document !== "undefined" && !document.getElementById("sheet-kit-css")) {
  ...
  s.textContent = [
    ...
    ".sk-shimmer{background:linear-gradient(90deg,#ebebe6 0%,#f5f5f1 50%,#ebebe6 100%);background-size:360px 100%;animation:sheetshimmer 1.4s ease-in-out infinite}",
  ].join("\n");
...
const SheetBtn = ({ kind = "primary", state = "default", icon, children, full, size = "md" }) => {
  ...
  const ring = "0 0 0 3px rgba(59,91,254,0.18)";
  const looks = {
    primary: { bg: MT.brand, fg: "#fff", sh: "0 1px 2px rgba(15,20,32,0.10)", hoverBg: shade(MT.brand, 0.9), pressBg: shade(MT.brand, 0.82) },
    secondary: { bg: "#fff", fg: MT.ink, border: "none", sh: MT_SH, hoverBg: "#f7f7f4", pressBg: "#efefec" },
    tinted: { bg: MT.brandTint, fg: MT.brandInk, hoverBg: "#dde4ff", pressBg: "#d0daff" },
...
const TextField = ({ label, value, placeholder, type = "text", state = "default", hint, message }) => {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 7, width: 248, opacity: disabled ? 0.55 : 1 }}>
      ...
      <div style={{ height: 50, padding: "0 16px", border: `1px solid ${borderC}`, borderRadius: 18, background: disabled ? "#f5f6f3" : "#fff", display: "flex", alignItems: "center", justifyContent: "space-between", gap: 8, boxShadow: focused ? "0 0 0 3px rgba(59,91,254,0.16)" : error ? "0 0 0 3px rgba(213,69,60,0.13)" : "none", fontSize: 15.5 }}>
        {value
...
const TextArea = ({ label, value, placeholder, state = "default" }) => {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 7, width: 248 }}>
      ...
      <div style={{ minHeight: 88, padding: "12px 16px", border: `1px solid ${focused ? MT.brand : MT.line}`, borderRadius: 18, background: "#fff", boxShadow: focused ? "0 0 0 3px rgba(59,91,254,0.16)" : "none", fontSize: 14.5, lineHeight: 1.5, color: value ? MT.ink : "#a4abb7" }}>{value || placeholder}</div>
    </div>
  );
};
...
const SelectField = ({ label, value, open }) => (
  <div style={{ display: "flex", flexDirection: "column", gap: 7, width: 248 }}>
    {label && <label style={{ fontSize: 13, fontWeight: 600, color: "#303744" }}>{label}</label>}
    <div style={{ height: 50, padding: "0 14px 0 16px", border: `1px solid ${open ? MT.brand : MT.line}`, borderRadius: 18, background: "#fff", display: "flex", alignItems: "center", justifyContent: "space-between", boxShadow: open ? "0 0 0 3px rgba(59,91,254,0.16)" : "none", fontSize: 15.5, color: MT.ink }}>
...
const Checkbox = ({ checked, label, state = "default" }) => {
  ...
  return (
    <div style={{ display: "flex", alignItems: "center", gap: 10, opacity: disabled ? 0.45 : 1 }}>
      <span style={{ width: 22, height: 22, borderRadius: 6, border: checked ? "none" : `1.5px solid ${state === "focus" ? MT.brand : "#c4cbd6"}`, background: checked ? MT.brand : "#fff", boxShadow: state === "focus" ? "0 0 0 3px rgba(59,91,254,0.18)" : "none", display: "inline-flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
...
const Seg = ({ options, value, w = 248 }) => (
  <div style={{ display: "flex", background: "#e9e9e4", borderRadius: 999, padding: 3, gap: 2, width: w }}>
    {options.map((o) => {
      const on = o.k === value;
      return <div key={o.k} style={{ flex: 1, textAlign: "center", padding: "7px 0", borderRadius: 999, fontSize: 13.5, fontWeight: 700, letterSpacing: "-0.005em", cursor: "pointer", whiteSpace: "nowrap", color: on ? MT.ink : MT.ink2, background: on ? "#fff" : "transparent", boxShadow: on ? "0 1px 2px rgba(12,13,18,0.10)" : "none" }}>{o.l}</div>;
    })}
  </div>
);
...
const StatusPill = ({ status }) => {
  ...
  return <span style={{ display: "inline-flex", alignItems: "center", gap: 5, padding: "3px 9px 3px 7px", borderRadius: 999, background: s.bg, color: s.fg, fontSize: 11, fontWeight: 500, lineHeight: 1.4, whiteSpace: "nowrap" }}><span style={{ width: 5, height: 5, borderRadius: 999, background: s.dot }} />{s.label}</span>;
};
...
const Banner = ({ tone = "info", icon, title, text, action }) => {
  ...
  const m = map[tone];
  return (
    <div style={{ display: "flex", alignItems: "flex-start", gap: 11, padding: "13px 15px", background: m.bg, borderRadius: 18, width: 300 }}>
...
const Toast = ({ tone = "neutral", icon, text }) => {
  ...
  return (
    <div style={{ display: "inline-flex", alignItems: "center", gap: 10, padding: "13px 17px", background: "#0C0D12", borderRadius: 999, boxShadow: "0 10px 30px -10px rgba(12,13,18,0.5)", width: 300 }}>
...
const EmptyState = ({ icon, title, text, action }) => (
  <div style={{ display: "flex", flexDirection: "column", alignItems: "center", textAlign: "center", padding: "26px 24px", gap: 4 }}>
    <div style={{ width: 56, height: 56, borderRadius: 999, background: "#ecece7", display: "inline-flex", alignItems: "center", justifyContent: "center", marginBottom: 6 }}>
      <Icon name={icon} size={26} color="#9CA2AE" />
    </div>
...
const Dialog = ({ title, body, primary, secondary, destructive }) => (
  <div style={{ width: 300, background: "#fff", borderRadius: 26, boxShadow: "0 24px 60px -16px rgba(12,13,18,0.4)", overflow: "hidden" }}>

  ...
  return (
    <div style={{ display: "flex", justifyContent: mine ? "flex-end" : "flex-start" }}>
      <div style={{ maxWidth: "84%" }}>
        {ai && <div style={{ display: "inline-flex", alignItems: "center", gap: 5, fontSize: 10.5, fontFamily: MT.mono, color: MT.brand, marginBottom: 3, letterSpacing: "0.04em" }}><span style={{ width: 6, height: 6, borderRadius: 999, background: MT.sage }} />MEDIUM</div>}
        ...
        <div style={{ fontSize: 11, padding: "4px 6px 0", textAlign: mine ? "right" : "left", fontFamily: MT.mono, display: "flex", gap: 5, justifyContent: mine ? "flex-end" : "flex-start", alignItems: "center",
...
const CGroup = ({ msgs, meta }) => (
  <div style={{ display: "flex", justifyContent: "flex-start" }}>
    <div style={{ maxWidth: "84%", display: "flex", flexDirection: "column", gap: 3 }}>
      ...
      <div style={{ fontSize: 11, color: MT.ink3, padding: "3px 6px 0", fontFamily: MT.mono }}>{meta}</div>
    </div>
  </div>
);
...
const CStatusRow = ({ mode = "ai" }) => {
  ...
  const aiOn = mode === "ai";
  return (
    <div style={{ display: "flex", alignItems: "center", gap: 9, padding: "9px 16px", background: m.tint, borderBottom: `1px solid ${MT.line}`, flexShrink: 0 }}>
...
const CNotice = ({ tone = "warning", icon, text, action }) => {
  ...
  const m = map[tone];
  return (
    <div style={{ display: "flex", alignItems: "flex-start", gap: 9, padding: "10px 16px", background: m.bg, borderBottom: `1px solid ${m.bd}`, flexShrink: 0 }}>
...
const CComposer = ({ state = "default", draft }) => {
  if (state === "windowClosed") {
    return (
      <div style={{ background: "#fff", borderTop: `1px solid ${MT.line}`, padding: "12px 16px 14px", flexShrink: 0 }}>
        <div style={{ display: "flex", alignItems: "flex-start", gap: 9, padding: "11px 12px", background: "#f4f4f1", border: `1px solid ${MT.line}`, borderRadius: 12, marginBottom: 10 }}>
        ...
        <button style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 7, width: "100%", height: 44, background: MT.brandTint, color: MT.brandInk, border: "none", borderRadius: 999, fontFamily: MT.sans, fontSize: 14, fontWeight: 600, cursor: "pointer" }}><Icon name="bell" size={16} color={MT.brandInk} />Dërgo kujtesë me shabllon</button>
      </div>
    );
  }
  if (state === "revoked") {
    return (
      <div style={{ background: "#fff", borderTop: `1px solid ${MT.line}`, padding: "12px 16px 14px", flexShrink: 0 }}>
        ...
        <button style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 8, width: "100%", height: 44, background: "#25D366", color: "#fff", border: "none", borderRadius: 999, fontFamily: MT.sans, fontSize: 14, fontWeight: 600, cursor: "pointer" }}><WhatsAppMark size={17} />Rilidh WhatsApp</button>
      </div>
    );
  }
  ...
  return (
    <div style={{ display: "flex", gap: 10, padding: "12px 16px 14px", background: "#fff", borderTop: `1px solid ${MT.line}`, flexShrink: 0, alignItems: "center" }}>
      <div style={{ flex: 1, minHeight: 44, padding: "0 16px", border: `1px solid ${typed ? "#d9d9d3" : MT.line}`, borderRadius: 999, fontSize: 14.5, color: typed ? MT.ink : MT.ink3, display: "flex", alignItems: "center" }}>{draft || "Shkruaj një mesazh…"}</div>
      <button style={{ width: 44, height: 44, background: typed ? MT.brand : "#ecece7", border: "none", borderRadius: 999, display: "inline-flex", alignItems: "center", justifyContent: "center", flexShrink: 0, cursor: "pointer" }}>
        {pending ? <Spinner size={16} color="#fff" /> : <Icon name="send" size={17} color={typed ? "#fff" : "#9CA2AE"} />}
      </button>
    </div>
  );
};
...
const CThread = ({ patient, statusRow, notice, messages, composer }) => (
  <Screen>
    ...
    {statusRow}
    {notice}
    <div style={{ flex: 1, minHeight: 0, overflowY: "auto", padding: "16px", display: "flex", flexDirection: "column", gap: 11, background: MT.bodyBg }}>{messages}</div>
    {composer}
  </Screen>
);

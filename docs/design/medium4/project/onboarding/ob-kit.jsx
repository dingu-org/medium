  ...
  <div style={{ width: "100%", height: "100%", background: "#f5f7fa", display: "flex", flexDirection: "column", overflow: "hidden", fontFamily: MT.sans }}>
    <header style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "12px 18px 13px", borderBottom: `1px solid ${MT.sep}`, background: "#fff", flexShrink: 0 }}>
      <div style={{ display: "flex", alignItems: "center", gap: 8, minWidth: 70 }}>
        <svg width="22" height="22" viewBox="0 0 40 40" fill="none"><rect width="40" height="40" rx="10" fill="#3B5BFE" /><path d="M11 27V13h2.4l4.1 7.6 4.1-7.6h2.4v14h-2.2v-9.5l-3.6 6.6h-1.4l-3.6-6.6V27H11Z" fill="#fff" /><circle cx="29.5" cy="13.5" r="1.6" fill="#7CC4A8" /></svg>
...
const OInput = ({ value, placeholder, prefix, state = "default", large }) => {
  ...
  return (
    <div style={{ display: "flex", alignItems: "center", height: large ? 52 : 48, padding: "0 16px", border: `1px solid ${bc}`, borderRadius: 12, background: "#fff", boxShadow: focused ? "0 0 0 3px rgba(59,91,254,0.16)" : error ? "0 0 0 3px rgba(179,50,43,0.12)" : "none" }}>
...
const ORadio = ({ icon, title, sub, selected }) => (
  <button style={{ display: "flex", alignItems: "flex-start", gap: 12, padding: "15px 16px", width: "100%", textAlign: "left", cursor: "pointer",
    ...
    boxShadow: selected ? "0 0 0 3px rgba(59,91,254,0.12)" : "none" }}>
...
const OServiceRow = ({ title, dur, selected }) => (
  <button style={{ display: "flex", alignItems: "center", gap: 12, padding: "14px 16px", background: selected ? MT.brandTint : "#fff", border: `1px solid ${selected ? MT.brand : MT.line}`, borderRadius: 12, cursor: "pointer", textAlign: "left", boxShadow: selected ? "0 0 0 3px rgba(59,91,254,0.12)" : "none", width: "100%" }}>

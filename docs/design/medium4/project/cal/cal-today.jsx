...
const AttnCard = () => (
  <div style={{ ...cardStyle, padding: "16px 18px", marginBottom: 24 }}>
    ...
    <div style={{ display: "flex", gap: 10, marginTop: 14 }}>
      ...
      <Btn kind="secondary" style={{ height: 44, background: "#f1f1ee", boxShadow: "none" }}>Anulo</Btn>
    </div>
  </div>
);
...
const NextAppt = ({ a }) => (
  <div style={{ ...cardStyle, padding: "16px 18px", display: "flex", alignItems: "center", gap: 13, cursor: "pointer" }}>
    <div style={{ display: "flex", flexDirection: "column", alignItems: "flex-start", width: 64, flexShrink: 0 }}>
      <div style={{ fontFamily: MT.display, fontWeight: 600, fontSize: 26, color: MT.ink, letterSpacing: "-0.03em", fontVariantNumeric: "tabular-nums", lineHeight: 1, whiteSpace: "nowrap" }}>{a.time}</div>
      <div style={{ fontSize: 11.5, color: MT.ink3, fontFamily: MT.mono, marginTop: 5, whiteSpace: "nowrap" }}>{a.duration}</div>
    </div>
...
const TodayDefault = () => (
  <Screen>
    ...
    <Body>
      ...
      <div style={{ marginBottom: 24 }}><NextAppt a={{ time: "10:00", duration: "45 min", name: "Anila Hoxha", service: "Konsultë e parë", status: "confirmed" }} /></div>
      <Group title="Më vonë sot">
        <Row leading={<div style={{ fontFamily: MT.display, fontWeight: 600, fontSize: 14, color: MT.ink, letterSpacing: "-0.02em", fontVariantNumeric: "tabular-nums", width: 46, whiteSpace: "nowrap" }}>14:30</div>} title="Endi Kola" subtitle="Takim vijues · 30 min" accessory={<CalPill status="pending" />} />
      </Group>
    </Body>
    <Tabs active="today" />
  </Screen>
...
const TodayLoading = () => (
  <Screen>
    ...
    <Body>
      <Sk w="42%" h={11} style={{ margin: "8px 0 12px" }} />
      <div style={{ ...cardStyle, padding: 16, marginBottom: 24, display: "flex", flexDirection: "column", gap: 12 }}>
...
const TodayEmpty = () => (
  <Screen>
    <TopBar title="Sot" sub="E mërkurë · 6 maj" right={<BellButton />} />
    <Body>
      <div style={{ ...cardStyle, padding: "20px 18px", marginBottom: 24, display: "flex", alignItems: "center", gap: 13, background: MT.greenTint }}>
      ...
      <div style={{ display: "flex", flexDirection: "column", alignItems: "center", textAlign: "center", padding: "30px 28px 10px" }}>
        ...
        <div style={{ fontFamily: MT.display, fontSize: 18, fontWeight: 700, color: MT.ink, letterSpacing: "-0.02em", lineHeight: 1.3 }}>Asnjë takim sot</div>
        ...
        <Btn kind="tinted" style={{ marginTop: 16, height: 44 }}>Shiko javën</Btn>
      </div>
    </Body>
    <Tabs active="today" />
  </Screen>

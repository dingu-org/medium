...
- **New route `/today`** = the exception-first home (`cal/cal-today.jsx`, canvas *Today & Calendar* → `today/*`). Sections: "Kërkon vëmendjen tënde" (reminder no-replies, escalations), "Më pas" (next appointment), and "Më vonë sot". (An earlier draft also had a "Medium po menaxhon N biseda" strip — **dropped by design decision 2026-07-07**; do not build it.) Default landing after sign-in/onboarding changes from `/calendar` → `/today`.

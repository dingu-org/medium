...
- [ ] Install `drizzle-orm`, `drizzle-kit`, `postgres` (the lightweight `postgres-js` client).
- [ ] Configure `drizzle.config.ts` pointing at `lib/db/schema.ts` and the EU Supabase URL.
- [ ] Add `pnpm db:generate`, `pnpm db:migrate`, `pnpm db:studio` scripts.
- [ ] Enable `pgcrypto` extension in the first migration (used for token encryption from Phase 2).

### Schema (one migration per logical group keeps diffs reviewable)

- [ ] `pts` — id (uuid, FK to `auth.users`), email, practice_name, timezone, ai_name, ai_greeting, ai_escalation_keyword, retention_days (default 90), created_at.
- [ ] `whatsapp_connections` — pt_id, phone_number_id, waba_id, access_token_encrypted, tier, quality_rating, connected_at, status (pending|active|revoked).
- [ ] `patients` — pt_id, name, phone (E.164), wa_id, notes, created_at.
- [ ] `conversations` — pt_id, patient_id, channel, last_inbound_at, ai_active (default true), escalation_state, created_at. Unique(patient_id, channel).
- [ ] `messages` — pt_id, conversation_id, external_id (UNIQUE for idempotency), role (patient|ai|pt), channel, content, template_id (nullable), tokens_in/out (nullable), model (nullable), created_at.
- [ ] `appointments` — pt_id, patient_id, starts_at, ends_at, service_type, status (pending|confirmed|cancelled|no_show|completed|rescheduled), notes, created_at.
- [ ] `availability_rules` — pt_id, weekday (0–6), start_time, end_time.
- [ ] `blocked_periods` — pt_id, starts_at, ends_at, label.
- [ ] `message_templates` — pt_id, name, language, status (pending|approved|rejected), meta_id, body, last_status_at.
- [ ] `reminder_jobs` — pt_id, appointment_id, scheduled_for, inngest_run_id, status, created_at.
- [ ] `push_subscriptions` — pt_id, endpoint, keys (jsonb), user_agent, created_at.
- [ ] `events` — id, pt_id, type, payload (jsonb), occurred_at — append-only domain event log.
- [ ] `audit_log` — id, pt_id, actor (uuid|service), action, target_table, target_id, occurred_at.

### Indexes

- [ ] `messages.external_id` UNIQUE (idempotency).
- [ ] `conversations(pt_id, last_inbound_at DESC)` (chat list ordering).
- [ ] `appointments(pt_id, starts_at)` (calendar queries).
- [ ] `appointments(starts_at)` partial WHERE status IN (pending, confirmed) (reminder scan).
- [ ] `events(pt_id, occurred_at DESC)`.

### RLS policies (write SQL by hand — Drizzle won't generate these)

- [ ] Enable `ROW LEVEL SECURITY` on every tenant-scoped table.
- [ ] Policy template per table: `USING (pt_id = auth.uid())` for SELECT/UPDATE/DELETE; `WITH CHECK (pt_id = auth.uid())` for INSERT.
- [ ] Service role bypasses RLS (default); the tenancy helper is the guardrail when running as service role.
- [ ] Test cross-tenant query: log in as PT A, attempt `SELECT * FROM patients WHERE pt_id = '<PT-B-id>'` → returns 0 rows, not an error. Confirms isolation.

### Tenancy helpers (`lib/tenancy/`)

- [ ] `getAuthedClient(req)` — returns a Supabase client bound to the user's JWT; RLS is enforced automatically.
- [ ] `getServiceClient(ptId)` — returns a service-role client. **Must require ptId**; throws if called without one.
...
- [ ] On first sign-up, insert a row into `pts` keyed to `auth.users.id`.

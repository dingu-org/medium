...
- [x] C6 Billing lifecycle & surfaces — renewal-monitor cron (renew/grace/downgrade), retention clamp, identity/services guards, /settings/billing UI, onboarding plan step, landing pricing section, i18n (depends C2+C5 + designs). ⚠ Live POK checkout GATED behind `LIVE_PAYMENTS_ENABLED=false` + `TODO(pok-spike)` (GuestCheckoutForm deferred until valid creds + confirmed minor-unit factor); everything else live. Visual QA vs billing/*.jsx designs → C8.

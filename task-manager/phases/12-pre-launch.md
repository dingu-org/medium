...
- **Batches of parallel agents need a serial pass before they count as done** (2026-07-31, `audit/logic-feature-hardening`). Seven agents shared one local Postgres, so all but one skipped the suite; typecheck and lint both passed anyway and hid two things. A rewritten test had never been executed and failed on its own harness, and — the expensive one — two agents independently owned the same fact: one consolidated the appointment-time formatter into `lib/format/appointment-time.ts` while the other left an inline copy in `app/(dashboard)/chat/actions.ts`, so the same booking reached the patient as two different times depending on whether the PT tapped send or the reminder job fired. Nothing typechecks that. Budget a solo run-the-suite-plus-read-the-seams pass after any concurrent batch, and specifically grep for the helper each agent claims to have consolidated.

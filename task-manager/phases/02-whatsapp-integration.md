...
- [x] `POST` handler:
  ...
  - [x] Coexistence webhooks:
    ...
    - [x] `smb_message_echoes` mirrors text Business-app PT replies as `messages.role = 'pt'`, creates patient/conversation rows when needed, does not emit `message.received`, and resets a guarded 2-hour AI pause.
    - [ ] Mirror or visibly represent non-text `smb_message_echoes`; they are currently skipped, while Meta requires app-originated messages to be digested and displayed.
    - [x] `account_update` / `PARTNER_REMOVED` marks the connection revoked and emits `wa.connection.revoked`.
    - [ ] Treat Coexistence `ACCOUNT_OFFBOARDED` as a revocation; it is currently logged as an ignored `account_update` event.
...
- [x] "Connect WhatsApp Business app" button on `/settings` — runs `FB.login({ config_id, response_type:'code', override_default_response_type:true, extras:{ featureType:'whatsapp_business_app_onboarding', setup:{} } })`, captures `phone_number_id` when present plus `waba_id` and the finish event from the `WA_EMBEDDED_SIGNUP` postMessage, POSTs `{ code, phoneNumberId?, wabaId, mode }` where `mode` is derived from the finish event (`app/(dashboard)/settings/connect-whatsapp.tsx`, `whatsapp-signup.ts`). Embedded Signup v4 (`5777b0a`, 2026-08-14); `featureType` was dropped in that migration and restored 2026-09-04 with a source-level regression test. The same hosted rejection remained after deployment, so the selector was a real defect but not the whole cause; Meta's v4 Phone Number First UI can show the common number picker before it automatically branches on an eligible Business-app number.
- [ ] **Complete session logging.** The exact-origin listener captures successful `FINISH*` events, but `readSignupMessage` currently discards Meta's `CANCEL` payload. Preserve its `current_step`, `error_code`, `error_message`, and `session_id` in a safe diagnostic event/UI so the next hosted failure identifies the rejected eligibility check. This is diagnostic and cannot itself change Meta's flow.
...
- [ ] During the one-time sync, tell the customer to keep the WhatsApp Business app open, expose progress/failure, and confirm completion; current UI only says synchronization is starting.
- [x] Unique index on `whatsapp_connections.phone_number_id` (migration `0005`) backs the 409 path + the unambiguous webhook lookup.

> **Deprecation — migrated 2026-08-14 (`5777b0a`).** Meta retires Embedded Signup v2/v3 on 2026-10-15; the client is on v4: `extras` is `{ featureType: 'whatsapp_business_app_onboarding', setup: {} }` and the connection mode is derived from the popup's finish event. Only the v3-era `sessionInfoVersion` key was removed. Meta's current v4 page confirms that Business-app onboarding still uses the `feature_type` launch parameter, but a deployed retry with the restored selector still failed before completion. See the E2E item under Notes.
...
- [ ] **Confirmed Tech Provider or Solution Partner entitlement.** This is an explicit Coexistence requirement, not merely a production-readiness nicety. In each app, check **Use cases → WhatsApp → Customize → Tech Provider onboarding** and confirm that Meta marks the provider onboarding complete. The repository still records Business Verification and App Review / advanced access as pending; Graph reporting both WhatsApp permissions as `live` does not prove the Tech Provider onboarding page is complete. If the tracker is current, this is the strongest shared explanation for Preview and Production rejecting the same Business-app number.
- [ ] **App-level webhook field subscription.** `POST /<appId>/subscriptions` with `object=whatsapp_business_account` must list all five fields: `messages`, `account_update`, `history`, `smb_app_state_sync`, `smb_message_echoes`. The three coexistence fields are a documented prerequisite for `featureType: whatsapp_business_app_onboarding`; without them `syncWhatsappCoexistence` requests a sync whose callbacks never arrive and Business-app replies never mirror. Production has all five; the 2026-09-04 Graph audit found Preview has no `whatsapp_business_account` subscription at all. Shipping the handlers is not the same as subscribing. Verify with `GET /<appId>/subscriptions?access_token=<appId>|<appSecret>`.
- [x] **`NEXT_PUBLIC_META_CONFIG_ID` must be this app's current v4 Embedded Signup configuration.** Preview uses its sole current configuration `3156974558025510`; Production uses its sole current configuration `1970044560354739`. Older IDs recorded on 2026-08-14 have been deleted and must not be restored. Config IDs are not readable through the Graph API, so their settings must be checked by exact ID in the dashboard.
- [x] **Calling origin must be allowlisted** in App Settings → Basic → App Domains _and_ Facebook Login for Business → Allowed Domains for the JavaScript SDK. Currently `medium.emae.events` / `emae.events` only — Vercel preview domains (`*.vercel.app`) are absent and will fail the JS-SDK domain check.
...
- `syncWhatsappCoexistence` registered on `wa.connection.created` (`lib/inngest/functions/sync-whatsapp-coexistence.ts`):
  ...
  - [ ] Persist each Meta request ID immediately after that one-time request succeeds. Currently both IDs are written only after both calls complete; if contacts succeeds and history fails, an Inngest retry can repeat the one-time contacts request.
  - [ ] Enforce the 24-hour sync deadline with an operator alert and offboarding/recovery path; the deadline is stored but never acted on.
...
- Meta's Coexistence offboarding flow is customer-owned in WhatsApp Business app → Settings → Account → Business Platform. Medium's current Disconnect button only unsubscribes Medium from the WABA and revokes the local row; its copy must not imply that this fully disconnects Cloud API from the number.

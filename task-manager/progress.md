...
**Current phase:** Phase 2 WhatsApp integration in flight — Inngest event-bus wiring + token crypto helpers + webhook handler landed; Embedded Signup is the next slice.
**Days into build:** 2
...
| 2 | WhatsApp integration | ◐ In flight | Inngest client + `serve` handler wired; 4 Phase 2 events typed; token crypto helpers round-trip-tested against pgcrypto; webhook handler verifies HMAC, idempotently persists messages, bumps conversations, emits `message.received`. Embedded Signup and channel adapter still pending. |
...
- Phase 2 — WhatsApp integration. Inngest, token crypto, and webhook handler all landed. Next slice: Embedded Signup callback (`app/api/auth/meta-embedded/route.ts`) + "Connect WhatsApp" button on `/settings`.
